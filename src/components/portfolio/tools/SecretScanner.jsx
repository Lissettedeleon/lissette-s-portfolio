  ...
  return (
    <div>
      <textarea
        ...
        rows={8}
        className="w-full resize-y rounded-lg border border-gold/20 bg-noir/60 px-4 py-3 font-mono text-sm text-cream outline-none focus:border-gold/60"
      />
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <button
          onClick={handleScan}
          disabled={!input.trim()}
          className="inline-flex items-center gap-2 rounded-lg bg-gold px-5 py-2.5 text-sm font-semibold text-noir transition-opacity hover:opacity-90 disabled:opacity-40"
        >
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-rose-dusty/50 px-5 py-2.5 text-sm font-semibold text-rose-dusty transition-colors hover:bg-rose-dusty/10"
        >
      ...
      {hasScanned && (
        <div className="mt-6">
          {findings.length === 0 ? (
            <div className="flex items-center gap-3 rounded-lg border border-emerald-400/30 bg-emerald-400/5 px-4 py-3 text-sm text-emerald-300">
              <ShieldCheck className="h-5 w-5 shrink-0" />
              No secrets detected.
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-center gap-3 rounded-lg border border-gold/20 bg-noir/40 px-4 py-3 text-sm text-cream/80">
                ...
                {criticalCount > 0 && (
                  <span className="rounded-lg px-2.5 py-0.5 text-xs font-medium" style={{ color: SEVERITY_STYLE.Critical.color, background: SEVERITY_STYLE.Critical.bg }}>
                    {criticalCount} Critical
                  </span>
                )}
                {warningCount > 0 && (
                  <span className="rounded-lg px-2.5 py-0.5 text-xs font-medium" style={{ color: SEVERITY_STYLE.Warning.color, background: SEVERITY_STYLE.Warning.bg }}>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="rounded-lg px-2.5 py-0.5 text-xs font-medium" style={{ color: style.color, background: "rgba(0,0,0,0.2)" }}>
                          {f.severity}
                        </span>
                      </div>
              ...
              <div className="mt-6 rounded-lg border border-rose-dusty/25 bg-rose-dusty/5 px-4 py-4">
